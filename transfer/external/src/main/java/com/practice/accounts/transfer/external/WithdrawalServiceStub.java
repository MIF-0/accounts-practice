package com.practice.accounts.transfer.external;

import static com.practice.accounts.transfer.domain.WithdrawalService.WithdrawalState.COMPLETED;
import static com.practice.accounts.transfer.domain.WithdrawalService.WithdrawalState.FAILED;
...
public class WithdrawalServiceStub implements WithdrawalService {
  ...
  public void requestWithdrawal(
      WithdrawalId id, Address address, Money amount) { // Please substitute T with prefered type
    final var existing =
        requests.putIfAbsent(id, new Withdrawal(finalState(), finaliseAt(), address, amount));
    if (existing != null
        && !Objects.equals(existing.address, address)
        && !Objects.equals(existing.amount, amount))
      throw new IllegalStateException(
          "Withdrawal request with id[%s] is already present".formatted(id));
  }
  ...
  record Withdrawal(WithdrawalState state, long finaliseAt, Address address, Money amount) {
